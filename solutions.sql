...
SELECT trans_id, date, type, amount, account_id
FROM trans
WHERE account_id = 793
ORDER BY date DESC
LIMIT 10;
-- In the client table, of all districts with a district_id lower than 10, how many clients are from each district_id? Show the results sorted by the district_id in ascending order.
SELECT DISTINCT district_id, SUM(client_id)
FROM client
...
SELECT DISTINCT SUM(card_id), type
FROM card
GROUP BY type
ORDER BY type DESC;
...
-- In the loan table, for each day in December 1997, count the number of loans issued for each unique loan duration, ordered by date and duration, both in ascending o
SELECT DISTINCT duration, COUNT(loan_id)
FROM loan
